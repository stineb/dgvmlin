...
summary_table <- as.data.frame(c('ALL',"C only models" ,"CN coupled models","Veg Demo","not Veg Demo",'CLASSIC',
'IBIS',
'LPJ',
'ORCHIDEE',
'VISIT',
...
summary_table$CN_coupled<-c(NA,NA,NA,NA,NA,'No',
'No',
'No',
'Yes',
'No',
...
'No',
'Yes')
summary_table$Veg_Demo<-c(NA,NA,NA,NA,NA,'No',
'No',
'Yes',
'No',
'No',
...
desert_filter <- function(df){
  ...
  mutate(l_croot_cveg_star = dCroot / dcveg_star) # I forgot this above
...
gdf <- gdf %>%
  mutate(data = purrr::map(data, ~desert_filter(.)))
...
gg1 <- gdf %>%
  ...
  khroma::scale_fill_batlowW(transform = "log1p", reverse = TRUE) +
  xlim(-1, 2) + 
  ylim(-1, 2) +
  labs(
...
Quantile_result <- nice_gdf%>%
  ...
  bind_rows(
    ...
    gdf %>%
      ...
      mutate(data = purrr::map(data, ~slice_sample(., n = nsample,replace=T))) %>% # smallest set here
      unnest(data) %>%
      filter(modl %in% c('LPJ', "CABLE-POP","JULES", "ISBA-CTRIP", "LPJ-GUESS", "SDGVM"))%>%
...
havealook_spatial_map <- havealook%>%
   ...
   mutate(l_cveg_star_npp = ifelse(l_cveg_star_npp>2,2,l_cveg_star_npp),
    ...
    summarise(l_cveg_star_npp = median(l_cveg_star_npp))|> 
      group_by(lon, lat) %>%
      summarise(model_num=n(),
        l_cveg_star_npp_sd = IQR(l_cveg_star_npp),
                 l_cveg_star_npp = median(l_cveg_star_npp))%>%
   filter(model_num>8) #remove some pixels in desert and tundra
...
mean_map <- ggplot() +
  ...
  theme_void() +
  labs(
    title = expression(paste(bold('(c)  '),italic(L)[Cveg^"*"/NPP],'  as Median across models')) 
  )
...
sd_map <- ggplot() +
  ...
  theme_void() +
  labs(
    title = expression(paste(bold('(d)  '),italic(L)[Cveg^"*"/NPP],'  as IQR across models')) 
  )
...
filter(modl==list_of_modl[i])%>%
  ...
  mean_map<-ggplot() +
    ...
    theme_void() +
    labs(
      title = expression(paste(italic(L)[Cveg^"*"/NPP],'  as median across models')) 
    )
...
filter(!modl=='ALL') %>%
  ...
  group_by(lon,lat) %>%
   summarise(model_num=n(),
             l_npp_gpp_sd=IQR(l_npp_gpp),
             l_npp_gpp=median(l_npp_gpp))%>%
  filter(model_num>8) #remove some pixels in desert and tundra
...
mean_map <- ggplot() +
    ...
    theme_void() +
    labs(
      title = expression(paste(bold('(c)  '),italic(L)[NPP/GPP],'  as median across models')) 
    )
...
sd_map <- ggplot() +
    ...
    theme_void() +
    labs(
      title = expression(paste(bold('(d)  '),italic(L)[NPP/GPP],'  as IQR across models')) 
    )
...
gg1 <- gdf %>%
  mutate(data = purrr::map(data, ~slice_sample(., n = nsample))) %>%
  unnest(data) %>%  
  filter(croot_init > quantile(croot_init, 0.01, na.rm = TRUE))%>%
...
supernice_gdf<-gdf %>%
  mutate(data = purrr::map(data, ~slice_sample(., n = 5000))) %>%
  unnest(data)%>%
   dplyr::filter(!is.nan(dCroot), !is.nan(dcveg), !is.infinite(dCroot), !is.infinite(dcveg)) %>%
...
nice_gdf  <- gdf %>%
  ...
  bind_rows(
    ...
    gdf %>%
      mutate(n = purrr::map_int(data, ~nrow(.))) %>%
      mutate(data = purrr::map(data, ~slice_sample(., n = 5000))) %>% # smallest set here
      unnest(data) %>%
      mutate(modl = "ALL")
  ) %>%
  dplyr::filter(!is.nan(dCroot), !is.nan(dcveg), !is.infinite(dCroot), !is.infinite(dcveg)) %>%
...
summary_table <- left_join(summary_table,Quantile_result,by='modl')
openxlsx::write.xlsx(summary_table,file = here("fig/TRENDYV11_summarytablev3_20250717.xlsx"))
# Lables for Histogram  
Labels <- nice_gdf%>%
  mutate(l_larger_than_one = l_croot_cveg>1) %>%
  group_by(modl) %>%
...
filter(!modl=='ALL') %>%
  ...
  group_by(lon,lat) %>%
   summarise(model_num=n(),
             l_croot_cveg_sd=IQR(l_croot_cveg),
             l_croot_cveg=median(l_croot_cveg))%>%
  filter(model_num>5) #remove some pixels in desert and tundra
...
bymodel_map <- ggplot() +
  ...
  theme_void() +
  labs(
      title = expression(paste(italic(L)[Croot/Cveg],'  as median across models')) 
  ) +
  facet_wrap(. ~ modl, ncol = 3)
...
mean_map <- ggplot() +
    ...
    theme_void() +
    labs(
      title = expression(paste(bold('(c)  '),italic(L)[Croot/Cveg],'  as median across models')) 
    )
...
sd_map <- ggplot() +
    ...
    theme_void() +
    labs(
      title = expression(paste(bold('(d)  '),italic(L)[Croot/Cveg],'  as IQR across models')) 
    )
...
nice_gdf  <- gdf %>%
  ...
  filter(l_cleaf_cveg<100000, l_cleaf_cveg>-1000000) # to remove NA and infinite


Labels <- nice_gdf%>%
  mutate(l_larger_than_one = l_cleaf_cveg>1) %>%
  group_by(modl) %>%
  summarise(number_of_records=n(),
            percentage_larger_than_one = round(sum(l_larger_than_one)/number_of_records,digits =2),
            percentage_smaller_than_one = 1-percentage_larger_than_one)
...
nice_gdf %>%
ggplot() +
  geom_density(aes(x = l_cleaf_cveg, y = ..density..),fill = "grey70") +
  theme_classic() +
  geom_hline(yintercept = 0, linetype = "dotted") +
  geom_vline(xintercept = 1, linetype = "dotted") +
  xlim(-1, 2.5) +
...
ggsave(here("fig/hist_dcleaf_dCveg_trendy_bymodl.pdf"), width = 9, height = 8)
...
ggsave(bymodel_map,filename = here("fig/rootshootration_trendyv11_bymodelv2.pdf"),height=10, width = 10)
