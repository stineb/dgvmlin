...
```{r}
calc_steady_state <- function(c_t, dcdt, npp, rh = NA, alternative = FALSE){
  ifelse(
    alternative,
    c_t * npp / rh,
    c_t * 1/(1 - dcdt/npp)
...
get_steadystate <- function(df){
  ...
  df |>
    ...
    mutate(
      ...
      # XXX Beni: use formula as described in paper
      # cveg_star_final = calc_steady_state(
      #   cveg_final, 
      #   cveg_change, 
      #   npp_final, 
...
```{r}
# XXX unclear how this was used.
testdf <- gdf$data[[4]] |>
  mutate(dnpp = npp_final / npp_init)

# This should be raning from 0 to 50%
gdf$data[[1]]$dgpp %>%
  quantile(., probs = c(0.1, 0.5, 0.9), na.rm = TRUE)
```
...
print(format(quantile(gdf_sampled$l_npp_gpp, probs =  c(0.25, 0.5, 0.75), na.rm = TRUE), digits = 2))
...
gg_density <- gdf_sampled |> 
  ...
  geom_text(
    data = labels,
    aes(x = -Inf, y = Inf, label = percentage_smaller_than_one),
    hjust = -2, 
    vjust= 1.5) +
  geom_text(
    data = labels,
    aes(x = Inf, y = Inf, label = percentage_larger_than_one),
    hjust = 3,
    vjust= 1.5) +
  labs(
...
ggsave(
  bymodel_map, 
  filename = here("fig/Spatial_MAP_dnpp_dgpp_trendyv11_bymodel.pdf"),
  height = 10, 
  width = 10
  )
```
...
## XXX code gets stuck here
ggsave(filename = here("fig/combined_npp_gpp.pdf"), height = 5, width = 6)
ggsave(filename = here("fig/combined_npp_gpp.png"), height = 5, width = 6)
``` 
...
model_test<- gdf_long_spatial_map |> 
...
sd_map <- ggplot() +
  ...
  theme_void() +
  labs(
    title = expression(paste(italic(L)[Cveg^"*"/NPP],'  as SD across models')) 
  )
...
list_of_modl<-unique(gdf_long$modl)
bymodel_map<-list()
...
df_quantiles <- gdf_sampled |>
  group_by(modl) |>
  summarise(l_croot_cveg_50 = quantile(l_croot_cveg, probs=0.5)) 
...
```{r}
gdf_long <- gdf |> #Prepare the dataset, this is similar to the histogram above
  unnest(data)  |>
  dplyr::filter(!is.nan(dCroot), !is.nan(dcveg), !is.infinite(dCroot), !is.infinite(dcveg)) |>
  filter(l_croot_cveg<100000, l_croot_cveg>-1000000) |> # to remove NA and infinite
  mutate(lon=ifelse(lon>180,lon-360,lon)) #some maps have longitude from 0 to 360, need to convert to -180 to 180
...
ggsave(
  bymodel_map,
  filename = here("fig/Spatial_MAP_dcroot_dcveg_trendyv11_bymodel.pdf"),
  height=10, 
  width = 10
  )
...
sd_map <- ggplot() +
    ...
    theme_void() +
    labs(
      title = expression(paste(italic(L)[Croot/Cveg],'  as SD across models')) 
    )
...
ggsave(
  filename = here("fig/Spatial_MAP_dcveg_dcroot_trendyv11.pdf"),
  height=4, 
  width = 9
  )
```
