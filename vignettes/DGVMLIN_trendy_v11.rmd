...
labels <- gdf_sampled %>%
  ...
  summarise(n_recs = n(),
            perc_larger = round(sum(l_larger)/n_recs, digits = 2),
...
gg_density <- gdf_sampled %>% 
  ...
  geom_text(
    data = labels,
    aes(x = -Inf, y = Inf, label = perc_smaller),
    hjust = -2, 
    vjust= 1.5) +
  geom_text(
    data = labels,
    aes(x = Inf, y = Inf, label = perc_larger),
    hjust = 3,
    vjust= 1.5) +
  labs(
...
gdf_sampled %>%
  ...
  geom_vline(xintercept = 1, linetype = "dotted") +
  xlim(-1, 2.5) +
  geom_text(data = labels,aes(x = -Inf, y = Inf, label = perc_smaller), hjust = -2, vjust =  1.5) +
  geom_text(data = labels,aes(x = Inf, y = Inf, label = perc_larger), hjust = 3, vjust = 1.5)+
  labs(
    x = expression(paste(italic(L)[NPP/GPP]))
  ) +
  facet_wrap(. ~ modl, ncol = 3,  scales = "free") 
...
print(format(quantile(gdf_sampled$l_cveg_star_npp, probs =  c(0.25, 0.5, 0.75), na.rm = TRUE), digits = 2))
...
print(format(quantile(gdf_sampled$l_croot_cveg, probs =  c(0.25, 0.5, 0.75), na.rm = TRUE), digits = 2))
...
# Number for text
print("Quartiles of L_Croot:Cveg*")
print(format(quantile(gdf_sampled$l_croot_cveg_star, probs =  c(0.25, 0.5, 0.75), na.rm = TRUE), digits = 2))
