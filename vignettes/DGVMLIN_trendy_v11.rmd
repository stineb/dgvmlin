...
```{r cveg_star_cwood_bymodl, fig.height=9, out.width="100%"}
set.seed(1982)
...
ggsave(here("fig/dCwood_dcveg_bymodl.pdf"), width = 9, height = 8)
ggsave(here("fig/dCwood_dcveg_bymodl.png"), width = 9, height = 8)

# we need to get some stat for the overall excel table
df_quantiles <- gdf_sampled %>%
  group_by(modl) %>%
  summarise(l_cwood_cveg_50 = quantile(l_cwood_cveg, probs = 0.5, na.rm = TRUE)) 
...
gdf_sampled %>%
  ggplot() +
  geom_density(aes(x = l_cwood_cveg, y = ..density..), fill = "grey70") +
  theme_classic() +
  geom_hline(yintercept = 0, linetype = "dotted") +
  # geom_vline(xintercept = 0, linetype = "dotted") +
  geom_vline(xintercept = 1, linetype = "dotted") +
...
By model.
```{r cveg_star_cwood_bymodl_maps, fig.height=9, out.width="100%"}

gdf_mdlist<-gdf%>%
    unnest(data) %>%
dplyr::filter(dcwood>-100000)
plot_by_model <- function(use_modl, df){
...
plotlist <- purrr::map(
  as.list(unique(gdf_mdlist$modl)),
  ~ plot_by_model(., gdf)
)
...
ggsave(
  filename = here("fig/map_dCwood_dcveg_bymodel.pdf"),
  height = 5, 
  width = 10
  )
...
ggsave(here("fig/summary_table.pdf"), width = 8, height = 9)
```

##Summary table with cwood and croot
```{r summary_table, fig.height=9, out.width="80%"}
df <- summary_table |> 
  filter(modl != "ALL")

# Pivot to long format
column_order <- c("l_npp_gpp_50", "l_cveg_star_npp_50", "l_croot_cveg_50", "l_cleaf_cveg_star_50", "l_cwood_cveg_50","cn_coupled","Veg_Demo")
...
df_long2 <- df %>%
  group_by(cn_coupled) |> 
  summarise(across(all_of(c("l_npp_gpp_50", "l_cveg_star_npp_50", "l_croot_cveg_50", "l_cleaf_cveg_star_50", "l_cwood_cveg_50")), ~ median(.x, na.rm = TRUE))) |> 
  pivot_longer(-cn_coupled, names_to = "link", values_to = "value") |> 
  mutate(link = factor(link, levels = column_order)) |> 
  left_join(
    cn_id,
...
df_long4 <- df %>%
  ...
  mutate(link = factor(link, levels = column_order)) |> 
  mutate(modl = "     ALL")
df_long_all<-bind_rows(df_long,df_long2,df_long3,df_long4)

gg2 <- ggplot(df_long_all, aes(x = link, y = modl, fill = value)) +
  geom_tile(color = "white", show.legend = FALSE) +
  geom_text(aes(label = round(value, 2)), color = "grey30") +
  khroma::scale_fill_vik(
    name = expression(paste(italic(L))),
  ...
  scale_x_discrete(labels = column_labels, position = "top") +
  scale_y_discrete(limits = rev(unique(df_long_all$modl))) +
  coord_fixed(ratio = 0.5) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 0, hjust = 0.5),
...
ggsave(here("fig/summary_table_extended.pdf"), width = 8, height = 9)
