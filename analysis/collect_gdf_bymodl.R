collect_gdf_bymodl <- function(modl, dir, 
  ...
  gdf <- read_nc_onefile(paste0(dir, "/processed/", filn_cSoil_init, ".nc"), ignore_time = TRUE, varnam = "cSoil") %>% 
    ...
    ### cLitter
    left_join(
      read_nc_onefile(paste0(dir, "/processed/", filn_cLitter_init, ".nc"), ignore_time = TRUE, varnam = "cLitter") %>%
    ...
    left_join(
      read_nc_onefile(paste0(dir, "/processed/", filn_cLitter_final, ".nc"), ignore_time = TRUE, varnam = "cLitter") %>%
        nc_to_df(varnam = "cLitter") %>%
        tidyr::drop_na(myvar) %>%
    ...
    left_join(
      read_nc_onefile(paste0(dir, "/processed/", filn_landCoverFrac, ".nc"), ignore_time = TRUE, varnam = "landCoverFrac") %>%
