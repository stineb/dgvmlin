...
setwd('H:/Oxford/Chapter_four/Beni/DGVM_density_plot/trendy_v9')
```
...
df <- filnams %>% 
  ...
  ## get starting year
  left_join(read_csv("startyear_trendy_v9_S1.csv"), by = "modl") %>% 
...
### ORCHIDEE-CNP(no land cover change map)
model_no<-which(gdf$modl=="ORCHIDEE-CNP")
model_df<-gdf$data[[model_no]]
robustbase::colMedians(as.matrix(model_df),na.rm = T)
sum(!model_df$landCoverFrac==0)

### ORCHIDEEv3(no land cover change map)
model_no<-which(gdf$modl=="ORCHIDEEv3")
model_df<-gdf$data[[model_no]]
robustbase::colMedians(as.matrix(model_df),na.rm = T)
sum(!model_df$landCoverFrac==0)

### OCN(no land cover change map)
model_no<-which(gdf$modl=="OCN")
model_df<-gdf$data[[model_no]]
robustbase::colMedians(as.matrix(model_df),na.rm = T)
sum(!model_df$landCoverFrac==0)
gdf$data[[model_no]]$cwood_init<-gdf$data[[model_no]]$cveg_init - gdf$data[[model_no]]$cleaf_init - gdf$data[[model_no]]$croot_init
# the Cwood of this model are all 0 , I derive it from cVeg
gdf$data[[model_no]]$cwood_final<-gdf$data[[model_no]]$cveg_final - gdf$data[[model_no]]$cleaf_final - gdf$data[[model_no]]$croot_final
...
model_no<-which(gdf$modl=="JULES-ES-1p0")
model_df<-gdf$data[[model_no]]
robustbase::colMedians(as.matrix(model_df))
sum(!model_df$landCoverFrac==0)
...
### YIBs(no land cover change map)
model_no_YIBS<-which(gdf$modl=="YIBs")
model_df<-gdf$data[[model_no_YIBS]]
robustbase::colMedians(as.matrix(model_df),na.rm = T)
sum(!model_df$landCoverFrac==0)
#strange that na mask is not consistent for all variables 
#let's just drop na
#and there are negative NPP?
gdf$data[[model_no_YIBS]]<-gdf$data[[model_no_YIBS]]%>% 
  drop_na()%>%
  filter(npp_final>=0)%>%
  filter(npp_init>=0)
...
#save(gdf,file="gdf_Trendyv9_20220828.rda")
...
load("gdf_Trendyv9_20220828.rda")
